<template>
  <aside
    class="opacity-transition block bg-gray-100 mt-8 -mx-4 lg:bg-transparent lg:mt-0 lg:mx-0 lg:inset-0 z-90 lg:mb-0 lg:static lg:h-auto lg:overflow-y-visible lg:pt-0 lg:w-1/4 lg:block border-l pl-4"
  >
    <div
      ...
      >
        <div v-for="(sublinks, group) in sortedLinks" :key="`links-${group}`">
          ...
          <ul class="pb-8">
            ...
            >
              <NuxtLink
                class="p-2 pl-4 flex rounded hover:text-dalil-lightindigo dark:hover:text-dalil-lightindigo transition-colors duration-300 ease-linear"
                exact-active-class="text-dalil-lightindigo bg-indigo-100 dark:bg-indigo-800 dark:text-white"
                active-class="text-dalil-lightindigo dark:text-white"
                :to="toLink(link)"
              >
